...
def proceso(env, number, interval, cpu_ram_total, espera):
    # Genera Proceso random

    for i in range(number):
        p = new(env, 'Proceso %02d' % (i+1), cpu_ram_total, espera)  # crea proceso
        env.process(p)
...
def new(env, name, cpu_ram_total, espera):
    ...
    r = running(env, p_ram, cpu_ram_total, instruccion_proceso,
                espera, LISTA_TIEMPOS)  # se crea un proceso running (ver metodo abajo)
    env.process(r)  ## Se efectura el proceso previo

def running(env, p_ram, cpu_ram_total, instruccion_proceso, espera, LISTA_TIEMPOS):
    global tiempofinal
    global tiempo2
    global totalwait
    global suma
    ...
    while instruccion_proceso > 0:
        ...
        with cpu.request() as reqcpu:  # entra a cpu a ejecutar procesos
            yield reqcpu
            yield env.timeout(1)
            if instruccion_proceso > 6:  # si el proceso tiene mas de 3 instrucciones, se ejecutan 3
                instruccion_proceso = instruccion_proceso - 6

                yield env.timeout(1)

                if decision == 2:  # si despues de ejecutar sale esperar , el proceso se ejecuta hasta que salga siguiente
                    with espera.request() as reqwait:
                        yield reqwait
...
totalwait = 0
env.process(proceso(env, PROCESOS_A_REALIZAR, INTERVALO, cpu_ram_total, espera))
env.run()
